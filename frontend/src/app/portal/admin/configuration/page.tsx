        ...
        <div className="flex flex-col gap-xs">
          {SECTIONS.map((section) => (
            <button
              ...
              className={`text-left px-md py-sm rounded-lg text-sm font-semibold transition-colors ${activeSection === section ? 'bg-primary text-on-primary' : 'text-on-surface-variant hover:bg-surface-container-low'}`}
            >
              {section}
            </button>
          ))}
            ...
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-md">
                <div>
                  ...
                  <input value={orgName} onChange={(e) => setOrgName(e.target.value)} className="w-full bg-surface-container border border-outline-variant rounded-lg px-sm py-sm text-sm outline-none focus:border-primary" />
                </div>
                <div>
                  <label className="text-xs font-semibold text-on-surface-variant mb-xs block">Default Language</label>
                  <select value={language} onChange={(e) => setLanguage(e.target.value)} className="w-full bg-surface-container border border-outline-variant rounded-lg px-sm py-sm text-sm outline-none focus:border-primary">
                ...
                <div>
                  ...
                  <select value={dateFormat} onChange={(e) => setDateFormat(e.target.value)} className="w-full bg-surface-container border border-outline-variant rounded-lg px-sm py-sm text-sm outline-none focus:border-primary">
                ...
                <div>
                  ...
                  <select value={timezone} onChange={(e) => setTimezone(e.target.value)} className="w-full bg-surface-container border border-outline-variant rounded-lg px-sm py-sm text-sm outline-none focus:border-primary">
                ...
                <div className="md:col-span-2">
                  ...
                  <input type="email" value={primaryContact} onChange={(e) => setPrimaryContact(e.target.value)} className="w-full bg-surface-container border border-outline-variant rounded-lg px-sm py-sm text-sm outline-none focus:border-primary" />
                </div>
                <div className="md:col-span-2">
                  <label className="text-xs font-semibold text-on-surface-variant mb-xs block">Organization Logo</label>
                  <div className="flex items-center gap-md">
                    <div className="w-16 h-16 rounded-xl bg-primary flex items-center justify-center">
                      <span className="material-symbols-outlined text-on-primary text-[28px]">corporate_fare</span>
                    </div>
                    <label className="px-md py-sm rounded-full border border-outline-variant text-sm font-semibold cursor-pointer hover:border-primary transition-colors">
                      Upload Logo
              ...
              <div className="border-t border-outline-variant/20 pt-lg">
                <h4 className="text-base font-semibold text-primary mb-md">System Toggles</h4>
                  ...
                  {[
                    ...
                    <div key={label} className="flex items-center justify-between p-sm rounded-lg hover:bg-surface-container-low">
                      ...
                      <button
                        ...
                        className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${state ? 'bg-primary' : 'bg-outline-variant'}`}
                      >
              ...
              <div className="flex justify-end pt-md border-t border-outline-variant/20">
                <button onClick={handleSave} className={`px-lg py-sm rounded-full text-sm font-semibold transition-colors ${saved ? 'bg-primary-fixed text-on-primary-fixed' : 'bg-secondary-container text-on-secondary-container hover:opacity-90'}`}>
